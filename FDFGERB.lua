...
send(chat,msg.id_,"\n•تم رفع الملف بنجاح وتفعيل المجموعات\n• ورفع الامنشئين الاساسين  والمنشئين  والمدراء والادمني بنجاح")   
end
...
send(msg.chat_id_, msg.id_,"اهلا عزيزي \n يمكنك ارسال ملصق او صوره او متحركه او فيديو او رساله ") 
return false
end 
...
list = {"ڄ","ك","که","پی","خسته","برم","راحتی","بیام","بپوشم","كرمه","چه","چ","ڬ","ٺ","چ","ڇ","ڿ","ڀ","ڎ","ݫ","ژ","ڟ","ݜ","ڸ","پ","۴","زدن","دخترا","دیوث","مک","زدن","😀","😃","😄","😁","😆","😅","😂 ","🤣","☺️","😊","😇","🙂","🙃","😉","😌","😍","🥰","😘","😗","😙","😚","😋","😛","😝","😜","😜","🤪","🤨","🧐","🤓","😎","🤩","🥳","😏","😒","😞","😔","😟","😕","🙁","☹️","😣","😖","😫","🥺","😢","😭","😤","😠","😡","🤬","🤯","😳","🥵","🥶","😱","😨","😰","😥","😓","🤗","🤔","🤭","🤫","🤥","😶","😐","😑","😬","🙄","😯","😦","😧","😮","😲","🥱","😴","🤤","😪","🤤","😵","🤠","🤑","🤕","🤒","😷","🤮","😷","🤢","🥴"}
for k,v in pairs(list) do
if string.find(text,v) ~= nil then
DeleteMessage(msg.chat_id_,{[0] = msg.id_}) 
return false
...
if result.sender_user_id_ == tonumber(1376155953) then
send(msg.chat_id_, msg.id_, "•  لا يمكنك حظر Source Manger \n")
return false 
end
...
if result.id_ == tonumber(1376155953) then
send(msg.chat_id_, msg.id_, "•  لا يمكنك حظر Source Manger \n")
return false 
end
usertext = '\n•العضو ⇠ ['..result.title_..'](t.me/'..(username or 'hlIl3')..')'
...
if userid == tonumber(1376155953) then
send(msg.chat_id_, msg.id_, "• لا يمكنك كتم SourceManger \n")
return false 
end
if tonumber(userid) == tonumber(bot_id) then  
...
tdcli_function({ID = "GetChannelMembers",channel_id_ = msg.chat_id_:gsub("-100",""), offset_ = 0,limit_ = 400},function(ta,nightrang)
t = ''
local list = nightrang.members_
for i=0 ,#list do
tdcli_function ({ID = "GetUser",user_id_ = nightrang.members_[i].user_id_},function(arg,data) 
if data.first_name_ ~= false then
username = ('t.me/'..data.username_ or 'tg://user?id='..nightrang.members_[i].user_id_)
...
local list = database:smembers(bot_id..'Constructor:User'..msg.chat_id_)
t = "\n• قائمة منشئين \n · · • • • ⍒ • • • · ·  \n"
for k,v in pairs(list) do
local username = database:get(bot_id.."user:Name" .. v)
if username then
...
local list = database:smembers(bot_id..'Manager:User'..msg.chat_id_)
t = "\n• قائمة المدراء \n · · • • • ⍒ • • • · ·  \n"
for k,v in pairs(list) do
local username = database:get(bot_id.."user:Name" .. v)
if username then
...
if text == 'تعطيل وضع الافتارات' and msg.reply_to_message_id_ == 0 then
if not Mod(msg) then
send(msg.chat_id_,msg.id_,'عذرا الامر يخص ( الادمن - مدير - منشئ )')
return false
end 
database:set(bot_id..'lock:avatark'..msg.chat_id_,true)   
...
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data) 
send(msg.chat_id_, msg.id_,'• بواسطه ⇠ ['..utf8.sub(data.first_name_,0,60)..'](T.ME/'..(data.username_ or 'hlIl3')..') \n• تـم تعطيل وضع الافتارات\n')  
end,nil)   
end
if text == 'تفعيل وضع الافتارات' and msg.reply_to_message_id_ == 0 then
if not Mod(msg) then
send(msg.chat_id_,msg.id_,'عذرا الامر يخص ( الادمن - مدير - منشئ )')
return false
end 
database:del(bot_id..'lock:avatark'..msg.chat_id_,true)   
list ={"lock:Bot:kick","lock:user:name","lock:Link","lock:forward","lock:Sticker","lock:Animation","lock:Video","lock:Fshar","lock:Fars","Bot:Id:Photo","lock:Audio","lock:vico","lock:Unsupported","lock:Markdaun","lock:Contact","lock:Spam"}
for i,lock in pairs(list) do 
database:del(bot_id..lock..msg.chat_id_,'del')    
end
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data) 
send(msg.chat_id_, msg.id_,'• بواسطه ⇠ ['..utf8.sub(data.first_name_,0,60)..'](T.ME/'..(data.username_ or 'hlIl3')..') \n• تـم تفعيل وضع الافتارات\n')  
end,nil)   
end
...
local nightrang = '• عدد الادمنيه : '..data.administrator_count_..
'\n\n عدد المطرودين : '..data.kicked_count_..
'\n\n• عدد الاعضاء : '..data.member_count_..
'\n\n• عدد رسائل القروب : '..(msg.id_/2097152/0.5)..
'\n\n• اسم المجموعه : ['..ta.title_..']'
send(msg.chat_id_, msg.id_, nightrang) 
end,nil)
end,nil)
end 
...
local photps = (nightrang.total_count_ or 0)
local rtpg = rtpa
local tahaa = {
"",
}
...
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, nightrang.photos_[0].sizes_[1].photo_.persistent_id_,get_id_text)       
else 
...
if result.status_.ID == "UserStatusRecently" and result.profile_photo_ ~= false then
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, nightrang.photos_[0].sizes_[1].photo_.persistent_id_,'\n✧ 𝘜𝘚𝘌 ➻  ['..username..']\n✧ 𝘔𝘚𝘎 ➻  '..Msguser..'\n✧ 𝘚𝘛𝘈 ➻ '..Rutba(msg.sender_user_id_,msg.chat_id_)..'\n✧ 𝘐𝘋 ➻  '..msg.sender_user_id_..'')
else 
if result.status_.ID == "UserStatusEmpty" and result.profile_photo_ == false then
send(msg.chat_id_, msg.id_,'\n\n✧ 𝘜𝘚𝘌 ➻  ['..username..']\n✧ 𝘔𝘚𝘎 ➻  '..Msguser..'\n✧ 𝘚𝘛𝘈 ➻ '..Rutba(msg.sender_user_id_,msg.chat_id_)..'\n✧ 𝘐𝘋 ➻  '..msg.sender_user_id_..'')  
else
...
nightrang = text:match("^اضف رسائل (%d+)$")
database:set(bot_id..'id:user'..msg.chat_id_,nightrang)  
...
nightrang = text:match("^اضف نقاط (%d+)$")
database:set(bot_id..'idgem:user'..msg.chat_id_,nightrang)  
...
{
{text = 'قفل 123', callback_data="/lockava"},{text = 'فتح 123', callback_data="/unlockava"},
},
...
if text and text:match('^كشف حساب (%d+)$') then
local id = text:match('^كشف حساب (%d+)$')
local text = 'اضغط لمشاهده الحىساب'
...
nightrangn = '['..(database:get(bot_id.."Chen:User:Name"..msg.sender_user_id_) or '')..']'
local nightrang ={ 
' ',
}
send(msg.chat_id_,msg.id_,nightrang[math.random(#nightrang)])
